...
App.config(['$stateProvider', '$urlRouterProvider',
    function ($stateProvider, $urlRouterProvider) {
        ...
        $stateProvider
			...
			.state('custompage', {
                url: '/custom/:partial',
                templateUrl: 'assets/views/custompage.html',
			...
			.state('customAngularPage', {
                url: '/customang/:partial',
                templateUrl: 'assets/views/custom/clientsang.html',
				controller: 'CustomAngularPageCtrl',
				resolve: {
                    deps: ['$ocLazyLoad', function($ocLazyLoad) {
                        return $ocLazyLoad.load({
                            insertBefore: '#css-bootstrap',
			...
			.state('clients', {
                url: '/clients',
                templateUrl: 'assets/views/partials/clients.html',
...
// Run our App
App.run(function($rootScope,$state, uiHelpers) {
    // Access uiHelpers easily from all controllers
    $rootScope.helpers = uiHelpers;
	//added for c46 to detect state
	//might need to change signature if minification fails
	
	$rootScope.uiState=$state;
	//alert("state is "+JSON.stringify($rootScope.uiState));
...
App.controller('AppCtrl', ['$scope','$rootScope', '$localStorage', '$window',
    function ($scope, $rootScope, $localStorage, $window) {
		...
		//added by c46 - abhijit to facilitate demostration could be morphed into offline flag to operate in offline mode
		$rootScope.DemoFlag=1;
...
// Header Controller
App.controller('HeaderCtrl', ['$scope','$state','AuthService', '$localStorage', '$window',
    function ($scope,$state,AuthService, $localStorage, $window) {
        // When view content is loaded
        $scope.$on('$includeContentLoaded', function () {
		...
		$scope.Logout=function () {
			...
			AuthService.Logout().then(function(){  
				...
				function(err){
					alert("Logout err is "+JSON.strigify(err));
				}
			);
		}	
    }
