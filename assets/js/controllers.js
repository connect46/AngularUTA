 ...
 *  Author     : Abhijit Bhattacharya for Connect46
 *  Description: Application controllers
 *
 */

// Login controller
App.controller('LoginCtrl', ['$scope', '$rootScope','$localStorage','$state','$stateParams','AuthService','AppUser', '$window',function ($scope,$rootScope, $localStorage,$state,$stateParams, AuthService, AppUser,$window) {
		...
		$scope.password="",
         ...
         $scope.Login = function () {
			 if ($rootScope.DemoFlag==0){
				 AuthService.Login({email: $scope.email, password: $scope.password}).then(function(response) {
					//alert(JSON.stringify(response));
					$state.go('custompage',{partial: 'dashboard'});
				},
				function(err){
 ...
 App.controller('CustomPageCtrl', ['$scope', '$localStorage','$state','$stateParams','$sce', '$window',function ($scope, $localStorage,$state,$stateParams,$sce, $window) {
	 
        $scope.CustomPageUrl=$sce.trustAsResourceUrl('assets/views/custom/'+$stateParams.partial+'.html');
		$scope.oneui.settings.sidebarMini = true;
		$scope.LaunchPage = function (myPartial) {
			 $state.go('custompage', {
                partial: myPartial
            });
		 };
		//copied from data tables controller
		// Init full DataTable, for more examples you can check out https://www.datatables.net/
        var initDataTableFull = function() {
			//alert('in datatables load function for full');
        ...
        var bsDataTables = function() {
			//alert('in boot strap datatables function');
    ...
    var initChartsSparkline = function(){
		...
		var jquery_chk=jQuery('.js-slc-bar1');
		...
		jquery_chk=jQuery('.js-slc-line1');
		if ((jquery_chk!=null) && (jquery_chk.length>0)){
		...
		jquery_chk=jQuery('.js-slc-line2');
		if ((jquery_chk!=null) && (jquery_chk.length>0)){
		...
		jquery_chk=jQuery('.js-slc-line3');
		if ((jquery_chk!=null) && (jquery_chk.length>0)){
        ...
        // Init jquery dependencies on load
		$scope.initCritical=function(){
			//data tables
			bsDataTables();
			initDataTableSimple();
			initDataTableFull();
		...
		//alert("page url is "+$scope.CustomPageUrl);
 }]);

 App.controller('CustomAngularPageCtrl', ['$scope', '$localStorage','$state','$stateParams','$sce', '$window',function ($scope, $localStorage,$state,$stateParams,$sce, $window) {
		...
		$scope.initData=function(){
			$resource('assets/data/'+$stateParams.partial+'.json').query().$promise.then(function(response) {
				alert("data is "+JSON.stringify(response[0]));
 ...
 App.controller('ClientsPageCtrl', ['$scope', '$rootScope','$resource','Patient','Gender','DTOptionsBuilder','$localStorage','$state','$stateParams','$sce', '$window',function ($scope, $rootScope,$resource, Patient,Gender,DTOptionsBuilder, $localStorage,$state,$stateParams,$sce, $window) {
        //$scope.CustomPageUrl=$sce.trustAsResourceUrl('assets/views/custom/'+$stateParams.partial+'.html');
		$scope.oneui.settings.sidebarMini = true;
		...
		$scope.LaunchPage = function (myPartial) {
		 ...
		 $scope.patients=[];
		 $scope.initData=function(){
			 //get clients from system if not a demo
			 if ($rootScope.DemoFlag==0){
				 //online so get list of patiens from application
				 //need a way to check for being logged in
				 if ($rootScope.currentUser!=null){
					 alert("rootscope uservalue is "+JSON.stringify($rootScope.currentUser));
				 } else {
					 alert("rootscope current user is null -expect error")
				 }
				 Patient.find()
				 .$promise
				.then(function(response) {
					//alert('first patient is '+JSON.stringify(response[0]));
				...
				function(err){
				 ...
				 //implement code for demo flag
				   $resource('assets/data/clients.json').query().$promise.then(function(response) {
					    //alert('');
						$scope.patients=response;
					});
			 }
